"""add preferences and activities

Revision ID: 3f8c3be63d5b
Revises: af54ba22a492
Create Date: 2020-05-18 16:08:48.104219

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '3f8c3be63d5b'
down_revision = 'af54ba22a492'
branch_labels = None
depends_on = None


def upgrade():
    ### commands auto generated by Alembic - please adjust! ###
    op.create_table('activity',
    sa.Column('name', sa.String(length=1000), nullable=False),
    sa.PrimaryKeyConstraint('name'),
    sa.UniqueConstraint('name')
    )
    op.create_table('preferences',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('day_start', sa.Integer(), nullable=True),
    sa.Column('day_end', sa.Integer(), nullable=True),
    sa.Column('temperature', sa.String(length=50), nullable=True),
    sa.Column('user_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('user_interests',
    sa.Column('activity_name', sa.String(length=1000), nullable=False),
    sa.Column('preferences_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['activity_name'], ['activity.name'], ),
    sa.ForeignKeyConstraint(['preferences_id'], ['preferences.id'], ),
    sa.PrimaryKeyConstraint('activity_name', 'preferences_id')
    )
    ### end Alembic commands ###


def downgrade():
    ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('user_interests')
    op.drop_table('preferences')
    op.drop_table('activity')
    ### end Alembic commands ###
